...
async fn process_psd(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<String, String> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        // Use the PSD's parent dir as the base for resolving relative paths,
        // but since we have absolute paths we use "/" as base.
        ...
        emit_log(
            &app_handle,
            &format!("PSD loaded ({:.1} MB)", psd_bytes.len() as f64 / 1_048_576.0),
        );
        ...
        // Build a modified config with the absolute output dir
        ...
        emit_log(&app_handle, &format!("Done! Output written to {}", output_dir.display()));
